...
- 当readable连接了数据源后，下游便可以调用readable.read(n)向流请求数据，同时监听readable的data事件来接收取到的数据。 ![stream-how-data-comes-out](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/stream-how-data-comes-out.png)

## 2. read(fs:2060,372)

read方法中的逻辑可用下图表示

![stream-read](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/stream-read.png)
