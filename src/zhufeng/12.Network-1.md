...
![分层模型](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/osi.png)
...
![tcpip](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/tcpip.png)
...
![分层模型](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/protocal.png)
...
![物理层](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/2.phisical.png)
...
![信号](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/8.mac.png)

### 3.2 互联网层(网络层)

- 位于传输层和网络接口层之间,用于把数据从源主机经过若干个中间节点传送到目标主机,并向传输层提供最基础的数据传输服务,它要提供路由和选址的工作

![网络层](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/8.iplayer.png)

#### 3.2.1 选址

交换机是靠MAC来寻址的，而因为MAC地址是无层次的,所以要靠IP地址来确认计算机的位置,这就是选址

![网络层](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/8.address.png)
...
![网络层](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/8.router.png)
...
![ipheader](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/ipheader.png)
...
![IP地址](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/9.ip.png)
...
![compose](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/ippart.png)

##### 3.2.3.4 IP地址表示

![compose](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/9.ip.png)
...
![compose](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/10.ipclass.png)
...
![transfer](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/10.mask.png)
...
![transfer](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/11.transfer.png)

#### 3.3.1 传输层的功能

提供了一种端到端的连接

![connect](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/11.connect.png)
...
![connect](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/tcpport.png)

- 源端口号和目标端口号，计算机通过端口号识别访问哪个服务,比如http服务或ftp服务，发送方端口号是进行随机端口，目标端口号决定了接收方哪个程序来接收 ![connect](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/tcpsequence.png)
- 32位序列号 TCP用序列号对数据包进行标记，以便在到达目的地后重新重装，假设当前的序列号为 s，发送数据长度为 l，则下次发送数据时的序列号为 s + l。在建立连接时通常由计算机生成一个随机数作为序列号的初始值 ![connect](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/tcpconfirm.png)
- 确认应答号 它等于下一次应该接收到的数据的序列号。假设发送端的序列号为 s，发送数据的长度为 l，那么接收端返回的确认应答号也是 s + l。发送端接收到这个确认应答后，可以认为这个位置以前所有的数据都已被正常接收。
- 首部长度：TCP 首部的长度，单位为 4 字节。如果没有可选字段，那么这里的值就是 5。表示 TCP 首部的长度为 20 字节。 ![connect](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/tcpcontrolpng.png)
...
![tcpack](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/tcpack.png)

- ACK(acknowledgement 确认)为1表示确认号 ![tcpsyn](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/tcpsyn.png)
- SYN(synchronous建立联机) 同步序号位 TCP建立连接时要将这个值设为1 ![tcpfin](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/tcpfin.png)
- FIN发送端完成位，提出断开连接的一方把FIN置为1表示要断开连接 ![tcpwindow](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/tcpwindow.png)
- 窗口值 说明本地可接收数据段的数目，这个值的大小是可变的。当网络通畅时将这个窗口值变大加快传输速度，当网络不稳定时减少这个值可以保证网络数据的可靠传输。它是来在TCP传输中进行流量控制的
- 窗口大小：用于表示从应答号开始能够接受多少个 8 位字节。如果窗口大小为 0，可以发送窗口探测。 ![connect](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/tcpcheck.png)
...
![sequencenumber](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/sequencenumber.jpg)
...
![sequencenumber](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/seqpre.png)
...
![connect](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/15.udp.png)

![connect](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/udplength.png)

![connect](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/udpcheck.png)
...
![域名](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/arealocal.png)
...
![dns](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/dnsquery.jpg)

- 客户端向本地域名服务器发出请求，我要访问[www.163.com，请告诉我它的IP地址](http://www.163.xn--com,ip-eu6jr5qee876bm8r511dm13cmsa/)
- 本地DNS服务器向DNS根服务器发出请求，根DNS服务器会告诉本地服务器(.com)的服务器地址
- 本地DNS服务器会向(.com域)发请求，会得到(163.com)的服务器地址
- 本地DNS服务器会向(163.com)发请求,会得到([www.163.com)的IP地址1.1.1.1](http://www.163.xn--com)ip1-w68lueu844a.1.1.1/)
- 本地DNS服务器向客户端回复域名([www.163.com)对应的IP地址是1.1.1.1](http://www.163.xn--com)ip1-w68lueo71cw9gxty2q2c.1.1.1/)

### 3.5 应用层

#### 3.5.1 协议

![connect](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/17.mail.png)
...
![connect](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/18.send.png)
...
![connect](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/19.receive.png)
...
![分层模型](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/reaenv2.png)
...
![sequencenumber](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/tcp_hand_shake_detail.png)

![sequencenumber](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/handshake.gif)

![sequencenumber](http://upload-markdown-images.oss-cn-beijing.aliyuncs.com/tcp_head.png)
